...
AudioDevice = (function(_super) {
  ...
  AudioDevice.prototype.start = function() {
    ...
    this._timer = setInterval(this.updateTime, 10);
...
WebAudioDevice = (function(_super) {
  ...
  function WebAudioDevice(sampleRate, channels) {
    ...
    this.bufferSize = Math.ceil(a / (this.deviceSampleRate / this.sampleRate) * this.channels);
    this.bufferSize += this.bufferSize % this.channels;
    if (this.deviceSampleRate !== this.sampleRate) {
      this.resampler = new Resampler(this.sampleRate, this.deviceSampleRate, this.channels, a * this.channels);
    }
    this.node = this.context[createProcessor](a, this.channels, this.channels);
